...
BOLT #7 `channel_update` メッセージに含まれる以下の4種類の手数料について、クエリモードに応じて行を選び、ヒストグラム・ECDF・散布図を生成します。
...
│   │   └── fee_snapshot_query.py     # クエリ組み立て（モード・期間フィルタ）
│   ├── analysis/
...
`LN_FEE_QUERY_MODE` で取得方式を切り替えます。

| モード | 説明 | 適するテーブル |
|--------|------|----------------|
| `latest_per_channel` | `DISTINCT ON (chan_id)` で、ORDER 時刻列が **最も新しい 1 行** | `channel_update`（行ごとに時刻が異なる） |
| `snapshot_batch` | スナップショット列が **全体の MAX** と一致する行のバッチ | スナップショット同期型テーブル |

**`--time-mode range` を付けたとき**

| モード | 挙動 |
|--------|------|
| `latest_per_channel` | まず ORDER 時刻列が **指定区間に入る行**に限定し、そのうえで **チャネルごとに区間内で最新の 1 行**を採用。 |
| `snapshot_batch` | スナップショット列が **区間内に入る値**のうち **最大**のスナップショット時刻に相当するバッチだけを採用。 |
