"""手数料など1列・2列を SELECT する SQL を組み立てる（チャネルごと最新1行のみ）。"""
...
# クエリにバインドする期間の両端（含む）。None のときは「全期間でチャネルごと最新」のみ。
...
def describe_time_filter_clause(time_range: QueryTimeRange) -> str:
...
def latest_per_channel_select_sql(
    ...
    *,
    ...
    channel_update のように行ごとに timestamp が異なるテーブル向け。
...
def fee_distribution_select_sql(
    ...
    *,
    ...
    """
    手数料（または ppm）1列を、チャネルごと最新1行から取得する。

    戻り値: (SQL, バインドパラメータのタプル, 人間向け説明)
    """
...
def fee_pair_distribution_select_sql(
    ...
    *,
    ...
    """
    base_fee と fee_rate など、2列をチャネルごと最新1行から取得する。
